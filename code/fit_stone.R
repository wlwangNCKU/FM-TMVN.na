...
sc.y = scale(Y.na)
Y_norm <- apply(Y.na, 2, function(x) {
  (x - min(x, na.rm = TRUE)) / (max(x, na.rm = TRUE) - min(x, na.rm = TRUE))
})
stonefit = function(Y, g, seed){
  p = ncol(Y)
  a=c(0,0,0,0,0)
  b=c(Inf, Inf,Inf,Inf,Inf)
  set.seed(seed)
  Y.knn = kNN(Y, k = 5)[,1:p]
  set.seed(seed)
...
s_1 = stonefit(Y = Y.na, g=1, seed=20250523)
s_2 = stonefit(Y = Y.na, g=2, seed=20250523)
s_3 = stonefit(Y = Y.na, g=3, seed=20250523)
s_4 = stonefit(Y = Y.na, g=4, seed=20250523)
s_5 = stonefit(Y = Y.na, g=5, seed=20250523)
s_6 = stonefit(Y = Y.na, g=6, seed=20250523)
total = list()
for (g in 1:6) {
